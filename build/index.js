...
function assertContiguousFiniteNumberArray(a) {
  if (!isContiguousFiniteNumberArray(a)) throw new Error("requires contiguous, finite, numeric data array --- found gaps or bad data");
}
  ...
  return function (sim) {
    ...
    const traces = chart.names.map(function (name, i) {
      ...
      try {
        ...
        if (typeof x[0] === 'number') assertContiguousFiniteNumberArray(x);
        if (typeof y[0] === 'number') assertContiguousFiniteNumberArray(y);
...
const helpers = {
    ...
    return function (sims, axis) {
      ...
      sims.forEach((sim, j) => {
        try {
          ...
          yMean[j] = stats.mean(rawData); // this is a standard error of the mean calculation
  ...
  smartPlotAgentProfits(chart) {
    ...
    return function (sims, axis) {
      ...
      const traces = agentIndexesForChart.map((agentIndex, k) => ({
        x: cases,
